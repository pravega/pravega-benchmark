...
controller_ip=$1
 ...
 do 
	...
	do 
		...
		pravega-benchmark/bin/pravega-benchmark http://$controller_ip:9090 -runtime 1 -eventspersec $packets -size $i -stream stream401 -producers 1  -blocking false -reporting 200 | tee   {$j}_thoroughput_{$i}_eventsize.txt
	 done
 done
