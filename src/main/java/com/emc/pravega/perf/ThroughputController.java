...
public class ThroughputController {
     ...
     */
    public ThroughputController(Instant startTime, int eventsPerSec) {
     ...
     */
    public ThroughputController(Instant startTime, long messageSize, double targetThroughput) {
        this(startTime, (int) ((targetThroughput * 1024 * 1024) / messageSize));
    }

    /**
     * @param eventsTillNow number of events produced till now.
     * @param endTime       timestamp of the most recently sent message
     * @return
     */
    public boolean needControl(long eventsTillNow, Instant endTime) {
        if (this.eventsPerSec <= 0) {
            // No need of limiting events
            return false;
        }

        final double elapsedSec = Duration.between(startTime, endTime).toMillis() / 1000.0;
        return elapsedSec > 0 && (eventsTillNow / elapsedSec) > this.eventsPerSec;
    }
    ...
    public synchronized void control() {
        ...
        if (timeNs >= MIN_SLEEP_NS) {
            ...
            try {
                ...
                timeNs -= Duration.between(sleepStart, Instant.now()).toNanos();
            }
        }
    }
}
